...
def add(request):
    ...
    if request.method == "POST":  # Check if the form was submitted
        ...
        if form.is_valid(): # Check if the form data is valid
            ...
            # Check for duplicate student_number ,  # Check if a student with the same student_number already exists
            if Student.objects.filter(student_number=new_student_number).exists():
                ...
                return redirect('add') # Redirect to the add page/ add.html if duplicate exists
            ...
            if grade_year < 1 or grade_year > 6:
                ...
                return render(request, 'add.html', {'form': form})
                            ...
                            # If no duplicate, extract cleaned data from the form
>>>>>>> test
...
def delete(request, id):
    ...
    return HttpResponseRedirect(reverse('index'))
=======
                'form': StudentForm(), # Create and save the new student object
                'success': True # Indicate successful addition of student
            })
        else: 
            messages.error(request, "There was an error with the form submission.") # Show an error message if the form is invalid
...
def edit(request, id):
    if request.method == "POST": # Check if the form was submitted
        ...
        form = StudentForm(request.POST, instance=student) # Bind form with POST data and the student's current data
        if form.is_valid(): # Check if the form data is valid
            form.save() # Save the changes to the student
            return render(request, 'edit.html', {
                'form': form,  # Display the form with the updated data
                'success': True # Indicate successful update
            })
    else:
        student = Student.objects.get(pk=id) # Retrieve student by ID
        form = StudentForm(instance=student) # Pre-fill the form with the student's existing data
    return render(request, 'edit.html', {
        'form': form, }) # Render the edit page with the form
